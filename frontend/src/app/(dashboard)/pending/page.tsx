  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard title="待审批" value={devices.length} hint="等待管理员确认接入" icon={Bell} tone="warning" />
        <StatCard title="审批权限" value="ReadWrite+" hint="批准或拒绝都会刷新相关列表" icon={ShieldAlert} tone="primary" />
        <StatCard
          title="同步状态"
          value={isFetching ? "同步中" : "已就绪"}
          hint="可手动刷新队列"
          icon={isFetching ? RefreshCw : Clock3}
          tone="neutral"
        />
      </div>
      ...
      <DashboardPanel
        title="接入申请"
        description="逐条核对设备标识、序列号后再执行审批操作。"
        action={
