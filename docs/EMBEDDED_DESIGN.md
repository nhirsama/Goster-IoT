...
| 类别        | STM32 引脚  | 连接对象                 | 模式/功能描述                    |
|-----------|-----------|----------------------|----------------------------|
| 通信 (UART) | PA9 (TX)  | ESP32-C3 GPIO 5 (RX) | 用于发送数据及唤醒 ESP32            |
| 通信 (UART) | PA10 (RX) | ESP32-C3 GPIO 6 (TX) | 用于接收 ESP32 的数据             |
| 传感器       | PA1       | DHT11 Data 引脚        | 单总线通信（需外接 4.7k/10k 上拉电阻）   |
| 模拟输入      | PA0       | 模拟信号源                | 使用 ADC1_IN0 通道，采样范围 0-3.3V |
| 输出元件 1    | PB0       | 推挽元件 A               | GPIO 推挽输出 (Push-Pull)      |
| 输出元件 2    | PB1       | 推挽元件 B               | GPIO 推挽输出 (Push-Pull)      |
| 公共地       | GND       | GND                  | 共地                         |
