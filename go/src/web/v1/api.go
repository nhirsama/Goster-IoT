...
type Deps struct {
	...
	CSRFStore         CSRFTokenStore     // CSRF token 存储
}
...
func New(deps Deps) *API {
	...
	api.csrfStore = deps.CSRFStore
	if api.csrfStore == nil {
		api.csrfStore = NewInMemoryCSRFStore()
	}
	return api
}
...
func (api *API) RegisterRoutes(mux *http.ServeMux) {
	...
	// 需要 CSRF 保护的写操作
	protectedWithCSRF := func(h http.HandlerFunc, minPerm inter.PermissionType) http.Handler {
		return api.Middleware(api.auth.LoadClientStateMiddleware(api.CSRFProtectionMiddleware(api.AuthMiddleware(h, minPerm))))
	}
...
func (api *API) RuntimeStore() interface{ GetUserCount() (int, error) } {
