...
func NewGatewayFromCoreWithConfig(ds inter.DataStore, registry inter.DeviceRegistry, presence inter.DevicePresence, downlinkCommands inter.DownlinkCommandService, l inter.Logger, cfg appcfg.APIConfig) inter.IoTGateway {
	return NewGatewayWithConfig(newLocalBackend(ds, registry, presence, downlinkCommands), l, cfg)
}

// NewGatewayFromCore 使用默认配置创建网络层服务。
func NewGatewayFromCore(ds inter.DataStore, registry inter.DeviceRegistry, presence inter.DevicePresence, downlinkCommands inter.DownlinkCommandService, l inter.Logger) inter.IoTGateway {
	return NewGatewayFromCoreWithConfig(ds, registry, presence, downlinkCommands, l, appcfg.DefaultAPIConfig())
}
