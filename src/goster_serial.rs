use crate::protocol_structs::{GosterHeader, SensorPacket};
use core::slice;
...
/// 编码 Goster 协议帧
pub fn encode_goster_frame<T: Serialize>(
    cmd_id: u16, 
    payload: &T, 
    seq: u64, 
    buffer: &mut [u8],
    ...
    // 1. 先序列化 Payload (使用 Postcard)
    // 直接序列化到 temp_buf 的偏移位置 (Header 之后)
    // Header 32 bytes.
    if temp_buf.len() < 32 + 16 + 5 { return Err(()); }
    
    // 我们先把 Payload 序列化到 temp_buf[32..]
    let payload_slice_len = match postcard::to_slice(payload, &mut temp_buf[32..]) {
        Ok(s) => s.len(),
        Err(_) => return Err(()),
    };
    let payload_len = payload_slice_len as u32;
    ...
    digest.update(&temp_buf[32..32+payload_slice_len]);
    let crc32 = digest.finalize();

    // 6. Footer (写入 temp_buf 末尾)
    let total_len = 32 + payload_slice_len + 16;
    if temp_buf.len() < total_len { return Err(()); }
    ...
    let footer_start = 32 + payload_slice_len;
    ...
    if buffer.len() < total_len + 5 { return Err(()); }
...
fn cobs_encode(input: &[u8], output: &mut [u8]) -> Result<usize, ()> {
    ...
    if output.len() < input.len() + 2 { return Err(()); }
...
pub fn cobs_decode(input: &[u8], output: &mut [u8]) -> Result<usize, ()> {
    ...
    while read_index < input.len() {
        ...
        if code == 0 { return Err(()); } // Zero byte found in data (should only be terminator if handled outside)
        
        // Copy code-1 bytes
        for _ in 0..(code - 1) {
            if read_index >= input.len() { return Err(()); } // Premature end
            if write_index >= output.len() { return Err(()); } // Output overflow
            output[write_index] = input[read_index];
            write_index += 1;
            read_index += 1;
        }
        ...
        if code < 0xFF && read_index < input.len() {
             if write_index >= output.len() { return Err(()); }
